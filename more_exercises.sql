...
FROM payment
WHERE SUBSTR(payment_date, 9 , 2) >= 27
;
...
SELECT payment_id, amount, payment_date
FROM payment
WHERE SUBSTR(payment_date, 6 , 5) = '05-27'
;
...
-- instead of the AND operator as in previous exercises.)

SELECT *
FROM payment
WHERE DATE(payment_date) IN  ('2005-05-25', '2005-05-29' , '2005-05-27') 
;
...
SELECT *
FROM film
WHERE LOWER(description) LIKE '%database%'
AND length > 180
;
...
AND LOWER(special_features) LIKE '%behind the scenes%'
LIMIT 10
;
...
actor.first_name AS actor_last_name, actor.last_name AS actor_last_name
FROM customer
LEFT JOIN actor ON customer.last_name = actor.last_name
;
